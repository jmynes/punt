...
import type { SortableHeaderCellProps, TicketTableHeaderProps } from './types'

/**
 * A sortable header cell for the ticket table.
 * Supports column reordering via drag-and-drop and sort toggling.
 */
function SortableHeaderCell({ column, sort, onToggleSort }: SortableHeaderCellProps) {
 ...
 */
function StaticHeaderCell({ column, sort, onToggleSort }: SortableHeaderCellProps) {
  const isSorted = sort?.column === column.id
  const handleSortClick =
    column.sortable && onToggleSort ? () => onToggleSort(column.id) : undefined
 ...
 * Supports optional column reordering and sorting.
 */
  ...
  return (
    <thead className="text-left text-xs text-zinc-500 uppercase tracking-wider">
      <tr className="border-b border-zinc-800/50">
        ...
        <th className="w-8" />
        {columns.map((column) => (
          <HeaderCell key={column.id} column={column} sort={sort} onToggleSort={onToggleSort} />
        ))}
      </tr>
    </thead>
  )
