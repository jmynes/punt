...
Tests that interact with the database (e.g., `database-backup*.test.ts`) share a single test database and cannot run in parallel. To prevent race conditions, the vitest config uses two projects:
